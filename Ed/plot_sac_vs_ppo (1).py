...
def plot_training(sac, ppo):
    ...
    ax.plot(smooth_steps(sac["steps"], W), smooth(sac["rewards"], W),
            color=COLOR_SAC, lw=2, label=f"SAC  (peak={max(sac['rewards']):.1f})")
    ax.plot(smooth_steps(ppo["steps"], W), smooth(ppo["rewards"], W),
            color=COLOR_PPO, lw=2, label=f"PPO  (peak={max(ppo['rewards']):.1f})")
    ...
    ax.plot(smooth_steps(sac["steps"], W), smooth(sac["losses"], W),
            color=COLOR_SAC, lw=2, label="SAC Critic Loss")
    ax.plot(smooth_steps(ppo["steps"], W), smooth(ppo["losses"], W),
            color=COLOR_PPO, lw=2, label="PPO Loss")
    ax.axhline(0, color="green", ls="--", alpha=0.5)
    ...
    ax.plot(smooth_steps(list(wt_sac), W), smooth(sac["rewards"], W),
            color=COLOR_SAC, lw=2, label=f"SAC  ({sac['wall_time']/60:.1f} min)")
    ax.plot(smooth_steps(list(wt_ppo), W), smooth(ppo["rewards"], W),
            color=COLOR_PPO, lw=2, label=f"PPO  ({ppo['wall_time']/60:.1f} min)")
    ax.axhline(0, color="green", ls="--", alpha=0.5)
...
# ══════════════════════════════════════════════════════════════════════════════
# PLOT 2 — Episode traces (3 plots, both models on same axes)
# ══════════════════════════════════════════════════════════════════════════════
def plot_traces(sac_ep, ppo_ep):
    ...
    # position
    ax = axes[0]
    ax.plot(ts_s, xs_s, color=COLOR_SAC, lw=1.5, label=f"SAC  [{res_s}]")
    ax.plot(ts_p, xs_p, color=COLOR_PPO, lw=1.5, label=f"PPO  [{res_p}]")
    ...
    ax.set_title("Position: A → B", fontweight="bold", pad=10)
    ax.legend(fontsize=9); ax.grid(alpha=0.3)

    # tilt
    ax = axes[1]
    ax.plot(ts_s, ths_s, color=COLOR_SAC, lw=1.5, label="SAC")
    ax.plot(ts_p, ths_p, color=COLOR_PPO, lw=1.5, label="PPO")
    ax.axhspan(-20, 20, alpha=0.06, color="green", label="Stable ±20°")
    ax.axhline(0, color="gray", alpha=0.4)
    ax.set_ylabel("Tilt Angle (°)")
    ax.set_title("Tilt Angle θ", fontweight="bold", pad=10)
    ax.legend(fontsize=9); ax.grid(alpha=0.3)

    # torque
    ax = axes[2]
    ax.plot(ts_s, torqs_s, color=COLOR_SAC, lw=1.5, label=f"SAC  (RMS={rms_s:.2f} Nm)")
    ax.plot(ts_p, torqs_p, color=COLOR_PPO, lw=1.5, label=f"PPO  (RMS={rms_p:.2f} Nm)")
